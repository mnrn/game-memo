...
auto async_write_message(boost::asio::ip::tcp::socket &socket,
  ...
  // As the return type of the initiating function is dedued solely from the
  // CompletionToken adn completion signature, we know that two different
  // asynchronous operations having the same completion signature will produced
  ...
  if (allow_partial_write) {
    // When delegating to the underlying operation we must take care to
    // perfectly forward completion token. This ensures that out operation works
    ...
    // As above, we must perfectly forward the completion token when calling the
    // alternate underlying operation.
