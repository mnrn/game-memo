...
auto async_write_message(boost::asio::ip::tcp::socket &socket,
  ...
  auto initiation = [](auto &&completion_handler,
    ...
    //   void ()
    // and the async_write operation has as completion handler signature of:
    //
    //   void(boost::system::error_code& error, std::size_t n)
    //
    // Both of these operations' completion handler signatures differ from out
    ...
    if (length == 0) {
      ...
      boost::asio::async_write(
          socket, boost::asio::buffer(message, length),
...
void test_callback() {
  ...
  async_write_message(socket, "", [](const boost::system::error_code &err) {
    if (!err) {
...
void test_future() {
  ...
  std::future<void> f =
      async_write_message(socket, "", boost::asio::use_future);
  io_context.run();
  try {
    f.get();
    std::cout << "Message sent" << std::endl;
